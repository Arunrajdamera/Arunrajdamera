<div align="center">

# Damera Arunraj

### Cybersecurity & Agentic AI Engineer

**Python • Agentic AI • LangGraph • RAG • FastAPI • Cybersecurity**

Building practical AI and cybersecurity systems with a focus on
**agentic workflows, intelligent automation, threat detection, and secure software engineering.**

<br/>

<a href="https://github.com/Arunrajdamera">
<img src="https://img.shields.io/badge/GitHub-Arunrajdamera-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

<a href="https://www.linkedin.com/in/arunrajdamera20/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>

<a href="https://dameraarunraj.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-Visit-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio"/>
</a>

<a href="mailto:arunrajdamera20@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>

</div>

---

## 👋 About Me

I'm a **B.Tech Computer Science Engineering – Cyber Security** undergraduate at
**GITAM (Deemed to be University), Visakhapatnam**, graduating in **2027**.

My current focus is at the intersection of **Agentic AI and Cybersecurity**, with
hands-on experience building backend systems, AI workflows, security applications,
and automation-oriented projects.

- 🤖 Building **Agentic AI systems** using Python and LangGraph
- 🧠 Exploring **RAG, AI agents, tool calling, and workflow orchestration**
- ⚡ Building backend services using **FastAPI, Flask, PostgreSQL, and Docker**
- 🛡️ Hands-on experience with **threat detection, log analysis, network security, and incident response**
- 🔎 Interested in **AI-powered security and intelligent automation**
- 🚀 Learning through practical projects, internships, security labs, and engineering practice

---

# 🎯 Current Focus

<table>
<tr>
<td width="25%" align="center">

### 🤖 Agentic AI

Multi-Agent Systems  
LangGraph  
AI Workflows  
Tool Calling

</td>
<td width="25%" align="center">

### 🧠 RAG & AI

RAG  
Vector Search  
Knowledge Retrieval  
LLM Applications

</td>
<td width="25%" align="center">

### ⚡ Backend

Python  
FastAPI  
PostgreSQL  
REST APIs

</td>
<td width="25%" align="center">

### 🛡️ Security

Threat Detection  
SIEM  
Incident Response  
Network Security

</td>
</tr>
</table>

---

# 🧰 Technical Skills

### Programming & Backend

`Python` `SQL` `FastAPI` `Flask` `REST APIs` `PostgreSQL`

### Agentic AI

`LangGraph` `RAG` `Multi-Agent Systems` `Tool Calling` `AI Workflows`

### AI & Data

`Machine Learning` `Vector Search` `pgvector`

### Cybersecurity

`Threat Detection` `SIEM Fundamentals` `Log Analysis` `Incident Response`
`Network Security` `Digital Forensics`

### Security Tools

`Wireshark` `Nmap` `Nessus` `Metasploit` `Burp Suite` `TryHackMe`

### Development & Infrastructure

`Git` `GitHub` `Docker` `Linux` `Windows`

---

# 🚀 Featured Projects

## 🤖 AeroMind — Agentic AI Drone Mission Platform

**Python • LangGraph • RAG • FastAPI • PostgreSQL • pgvector • Docker • LangSmith**

AeroMind is a **simulation-based Agentic AI platform** designed to demonstrate intelligent mission planning and decision-making through coordinated AI agents.

### What I Built

- Developed an agentic AI workflow for simulated drone mission operations.
- Designed specialized agents for **mission planning, perception, knowledge retrieval, risk analysis, and decision-making**.
- Used **LangGraph** to orchestrate agent workflows and manage state transitions.
- Implemented **RAG with PostgreSQL and pgvector** for contextual knowledge retrieval.
- Built **FastAPI REST APIs** for communication between the AI workflow and simulated drone operations.
- Added **human approval and safety checks** before high-risk simulated actions.
- Integrated **LangSmith** for agent workflow tracing and debugging.
- Used **Docker** to containerize and manage application services.

### Architecture

```text
User / Mission Request
        │
        ▼
Mission Orchestrator
        │
        ├───────────────┬───────────────┐
        ▼               ▼               ▼
 Mission Agent    Perception Agent   Knowledge Agent
        │               │               │
        └───────────────┼───────────────┘
                        ▼
                 Risk Analysis
                        │
                        ▼
                Decision / Action
                        │
                 Safety Validation
                        │
                        ▼
               Human Approval
                        │
                        ▼
              Simulated Execution
```

### Key Technologies

`Python` `LangGraph` `RAG` `PostgreSQL` `pgvector` `FastAPI` `Docker` `LangSmith`

> **Note:** AeroMind is a simulation/portfolio project. It does not control real drones, aircraft, flight controllers, or physical systems.

---

## 🛡️ AI-Powered Authentication & Intrusion Detection System

**Python • Flask • Machine Learning • SQL**

A security application designed to monitor authentication activity and identify potentially suspicious behavior.

### Highlights

- Developed a Python-based security monitoring application.
- Implemented a **Random Forest model** to identify suspicious authentication behavior.
- Achieved **88.8% accuracy** on the evaluated dataset.
- Added authentication and **role-based access control**.
- Implemented **audit logging** for security events.
- Built a dashboard for authentication and threat analysis.

### Security Capabilities

`Authentication Monitoring` `Threat Detection` `Audit Logging` `RBAC` `Machine Learning`

---

## ⚙️ Faculty Workload Allocation System — GITAM

**Python • FastAPI • PostgreSQL • Docker**

Backend system for automated faculty-subject allocation and workload management.

### Highlights

- Developed backend functionality using **Python and FastAPI**.
- Built REST APIs for faculty, subject, preference, and workload management.
- Used PostgreSQL for structured application data.
- Implemented allocation validation and workload verification.
- Added reporting functionality for allocation results.
- Used Docker for application deployment and service management.

### Engineering Focus

`REST APIs` `Database Design` `Validation` `Automation` `Backend Development`

---

## 🔐 Web Application Security Testing

**Nmap • Nessus • Metasploit • Burp Suite**

Hands-on security testing focused on identifying and documenting common web application and network vulnerabilities.

### Activities

- Network reconnaissance
- Vulnerability assessment
- Web application testing
- Security finding documentation
- Basic penetration testing workflows

---

# 💼 Experience

## Software Development Intern — GITAM University

**May 2026 – July 2026 | Visakhapatnam**

- Worked on a faculty workload allocation application using **Python, FastAPI, PostgreSQL, and Docker**.
- Developed and tested REST APIs for application functionality.
- Assisted with database integration and application validation.
- Implemented backend workflows for automated allocation and reporting.
- Worked with containerized development and deployment environments.

---

## Cybersecurity Intern — Cyber Crime Police Station

**January 2026 – March 2026 | Visakhapatnam**

- Assisted with analysis of **phishing and online fraud cases**.
- Supported **log and network traffic analysis** during investigations.
- Used Wireshark to examine network activity and suspicious communication.
- Assisted with **digital evidence documentation**.
- Supported technical investigation and reporting workflows.

---

# 📜 Certifications

### Cybersecurity

- 🎓 **Google Cybersecurity Professional Certificate**
- 🎓 **Cisco Ethical Hacker**
- 🎓 **Cisco Junior Cybersecurity Analyst Career Path**
- 🎓 **AWS Incident Response**
- 🎓 **Cybersecurity Analyst Job Simulation — Tata / Forage**

### Additional Training

- **Digital Forensics Basics — Cybrary**
- **TryHackMe — Hands-on Cybersecurity Training**

---

# 🧪 Currently Building & Exploring

```text
Agentic AI
    │
    ├── Multi-Agent Workflows
    ├── LangGraph
    ├── RAG
    ├── Tool Calling
    └── AI Decision Systems

Cybersecurity
    │
    ├── Threat Detection
    ├── SIEM & Log Analysis
    ├── Incident Response
    ├── Network Security
    └── AI for Security

Engineering
    │
    ├── Python
    ├── FastAPI
    ├── PostgreSQL
    ├── Docker
    └── REST APIs
```

---

# 📊 GitHub Statistics

<div align="center">

<a href="https://github.com/Arunrajdamera">
<img src="https://img.shields.io/github/followers/Arunrajdamera?style=for-the-badge&label=Followers&logo=github" alt="GitHub Followers" />
</a>

<a href="https://github.com/Arunrajdamera/Arunrajdamera">
<img src="https://img.shields.io/github/stars/Arunrajdamera/Arunrajdamera?style=for-the-badge&label=Profile%20Stars&logo=github" alt="Profile Repository Stars" />
</a>

<a href="https://github.com/Arunrajdamera/Arunrajdamera/commits/main/">
<img src="https://img.shields.io/github/last-commit/Arunrajdamera/Arunrajdamera?style=for-the-badge&label=Last%20Commit&logo=github" alt="Last Commit" />
</a>

<br/>
<br/>

<sub>Live GitHub metrics • Powered by GitHub repository data</sub>

</div>

---

# 🐍 Contribution Activity

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Arunrajdamera/Arunrajdamera/output/github-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Arunrajdamera/Arunrajdamera/output/github-snake.svg">
  <img src="https://raw.githubusercontent.com/Arunrajdamera/Arunrajdamera/output/github-snake.svg" alt="GitHub Contribution Snake" width="100%">
</picture>

</div>

---

# 🔍 Areas of Interest

`Agentic AI` `RAG` `Threat Detection` `SIEM & Log Analysis` `Incident Response` `Network Security` `Digital Forensics` `AI for Security`

---

# 🤝 Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-Arunrajdamera-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Arunrajdamera)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arunrajdamera20/)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://dameraarunraj.vercel.app/)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arunrajdamera20@gmail.com)

</div>
